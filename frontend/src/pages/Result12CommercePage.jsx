...
const Lightbox = ({ images, startIndex, onClose }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      onClick={onClose}
    >
      <button onClick={onClose} className="absolute top-6 right-6 w-12 h-12 rounded-full bg-white/10 border border-white/20 flex items-center justify-center text-white hover:bg-white/20 transition-colors z-10"><X size={20} /></button>
      <div className="absolute top-6 left-1/2 -translate-x-1/2 px-5 py-2 rounded-full bg-white/10 border border-white/20 backdrop-blur-md text-white text-[11px] font-black uppercase tracking-widest">{current + 1} / {images.length}</div>
      <button onClick={(e) => { e.stopPropagation(); prev(); }} className="absolute left-4 md:left-8 w-12 h-12 rounded-full bg-white/10 border border-white/20 flex items-center justify-center text-white hover:bg-white/20 transition-colors z-10"><ChevronLeft size={22} /></button>
      <AnimatePresence mode="wait">
        <motion.div key={current} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
          transition={{ duration: 0.3 }} className="px-16 max-w-5xl w-full max-h-[80vh] flex flex-col items-center gap-5" onClick={(e) => e.stopPropagation()}
        >
      ...
      <button onClick={(e) => { e.stopPropagation(); next(); }} className="absolute right-4 md:right-8 w-12 h-12 rounded-full bg-white/10 border border-white/20 flex items-center justify-center text-white hover:bg-white/20 transition-colors z-10"><ChevronRight size={22} /></button>
...
const FeaturedCarousel = ({ images, onImageClick }) => {
  ...
  return (
    <div className="relative overflow-hidden rounded-[2rem] shadow-2xl aspect-[16/9] md:aspect-[21/9] bg-gray-100 group">
      ...
      <button onClick={() => go(-1)} className="absolute left-4 top-1/2 -translate-y-1/2 w-11 h-11 rounded-full bg-black/40 backdrop-blur-md border border-white/15 flex items-center justify-center text-white hover:bg-black/60 transition-colors z-10 opacity-0 group-hover:opacity-100"><ChevronLeft size={20} /></button>
      <button onClick={() => go(1)} className="absolute right-4 top-1/2 -translate-y-1/2 w-11 h-11 rounded-full bg-black/40 backdrop-blur-md border border-white/15 flex items-center justify-center text-white hover:bg-black/60 transition-colors z-10 opacity-0 group-hover:opacity-100"><ChevronRight size={20} /></button>
    ...
    <div className="min-h-screen bg-white font-sans overflow-x-hidden">
      ...
      <section className="relative min-h-[55vh] flex items-end overflow-hidden bg-gray-950">
        ...
        <div className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16 pt-36">
          ...
          >Sunrise School · Academic Excellence</motion.p>
          ...
          >A visual celebration of our Commerce students' outstanding academic achievements.</motion.p>
        </div>
      </section>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-10">
            ...
            <h2 className="text-4xl md:text-5xl font-black uppercase tracking-tighter leading-none">Featured <span className="text-gray-200">Highlights</span></h2>
          </div>
      ...
      <section className="py-20 bg-gray-50 border-y border-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-12">
            ...
            <h2 className="text-4xl md:text-5xl font-black uppercase tracking-tighter leading-none">Complete <span className="text-gray-200">Gallery</span></h2>
      ...
      <section className="py-28 bg-white text-center relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-2xl mx-auto px-4">
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            ...
            <h2 className="text-4xl md:text-6xl font-black tracking-tighter uppercase leading-[0.85] mb-6">Be Part of Our <span className="text-gray-200">Legacy</span></h2>
            <p className="text-gray-500 font-light mb-10 text-lg">Join Sunrise School and add your name to our hall of excellence.</p>
